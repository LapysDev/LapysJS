...
(function() {
    ...
    with (LDK) with (Directives) {
        /* Class */
            /* [Property] Inspector ->> Evaluated by the `Functions.inspectFeature(...)` function. */
                ...
                Inspector.prototype = {
                    ...
                    evaluate: function evaluate(code) {
                        ...
                        if (identifier in object) {
                            // ... ->> Not inherited
                            while (code & LDKE.INHERITED) {
                                // ... --> Object.hasOwnProperty
                                if (LDKS.Object$prototype$hasOwnProperty) {
                                    if (false !== LDKF.functionCall(LDKN.Object$prototype$hasOwnProperty, object, 1, identifier)) break;
                                    throw new LDKE.Error(LDKE.INHERITED, "Expected `Error` constructor to be an accessible own-property value of the global object")
                                }

                                // ... --> Object.getOwnPropertyDescriptor
                                if (LDKS.Object$getOwnPropertyDescriptor) {
                                    if (undefined !== LDKN.Object$getOwnPropertyDescriptor(object, identifier)) break;
                                    throw new LDKE.Error(LDKE.INHERITED, "Expected `Error` constructor to be an accessible own-property value of the global object")
                                }

                                // ... --> [prototype]
                                if (LDKS.Object$prototype$__proto__ || LDKS.Object$getPrototypeOf) {
                                    var prototype = object;

                                    while (true) {
                                        prototype = LDKS.Object$prototype$__proto__ ? prototype.__proto__ : LDKN.Object$getPrototypeOf(prototype);

                                        if (null === prototype) break;
                                        if (identifier in prototype) throw new LDKE.Error(LDKE.INHERITED, "Expected `Error` constructor to be an accessible own-property value of the global object")
                                    }
                                ...
                                /* ... */
                                throw new LDKE.Error(0x0, "Support required for asserting own-property of objects i.e.: `Object.getOwnPropertyDescriptor`, `Object.getPrototypeOf, Object.prototype.__proto__`, or `Object.prototype.hasOwnProperty` features must be available (and un-modified) for assertion")
                            }

                            // ... ->> No getter/ setter
                            while ((code & LDKE.GETTER) || (code & LDKE.SETTER)) {
                                // ... --> __lookupGetter__
                                if (code & LDKE.GETTER) if (LDKS.Object$prototype$__lookupGetter__) {
                                    if (undefined === LDKF.functionCall(LDKN.Object$prototype$__lookupGetter__, object, 1, identifier)) break;
                                    throw new LDKE.Error(LDKE.GETTER, "Expected `Error` feature to not be an accessor, but otherwise a data value of the global object")
                                }

                                // ... --> __lookupSetter__
                                if (code & LDKE.SETTER) if (LDKS.Object$prototype$__lookupSetter__) {
                                    if (undefined === LDKF.functionCall(LDKN.Object$prototype$__lookupSetter__, object, 1, identifier)) break;
                                    throw new LDKE.Error(LDKE.SETTER, "Expected `Error` feature to not be a mutator, but otherwise a data value of the global object")
                                }

                                // ... --> Object.getOwnPropertyDescriptor, [prototype]
                                if (LDKS.Object$getOwnPropertyDescriptor || (LDKS.Object$prototype$__proto__ || LDKS.Object$getPrototypeOf)) {
                                    var prototype = object;

                                    for (var descriptor; null !== prototype; prototype = LDKS.Object$prototype$__proto__ ? prototype.__proto__ : LDKN.Object$getPrototypeOf(prototype)) {
                                        var ownDescriptor = LDKN.Object$getOwnPropertyDescriptor(prototype, identifier);

                                        if (undefined === ownDescriptor) {
                                            if (undefined === descriptor) continue;
                                            else break
                                        }

                                        descriptor = ownDescriptor;
                                        if (code & LDKE.GETTER) if ("get" in descriptor) throw new LDKE.Error(LDKE.GETTER, "Expected `Error` feature to not be an accessor, but otherwise a data value of the global object");
                                        if (code & LDKE.SETTER) if ("set" in descriptor) throw new LDKE.Error(LDKE.SETTER, "Expected `Error` feature to not be a mutator, but otherwise a data value of the global object")
                                    }
                                ...
                                /* ... */
                                throw new LDKE.Error(LDKE.GETTER | LDKE.SETTER, "Support required for asserting getter/ setter object property attributes i.e.: `Object.getOwnPropertyDescriptor, Object.getPrototypeOf`, `Object.prototype.__lookupGetter__`, `Object.prototype.__lookupSetter__` features must be available (and un-modified) for assertion")
                            }
                            ...
                            try { value = object[identifier] }
                                ...
                                throw new LDKE.Error(LDKE.INACCESSIBLE, "Expected `Error` constructor to be accessible via the global object")
                            }

                            // ... ->> User-defined assertions
                            for (var iterator = +0; iterator != this.assertions.length; ++iterator)
                            this.assertions[iterator](object, identifier, value)
                        }

                        else if (code & LDKE.MISSING) throw new LDKE.Error(LDKE.MISSING, "Expected `Error` constructor available within the global object");
            ...
            function StaticArray(length, maximum, fill, /* ... */ elements) {
                ...
                StaticArray.prototype = {
                    ...
                    length: +0, toString: null, valueOf: null
                };
