...
(function LoaderScreenScript(window = window, document = window.document, undefined = window.undefined || void 0) {
    ...
    let processingTime = 0,
        ...
        loaderScreenBorderAlpha = typeof window.loaderScreenBorderAlpha != 'undefined' ? loaderScreenBorderAlpha : !1,
        loaderScreenBorderTransparency = typeof window.loaderScreenBorderTransparency != 'undefined' ? loaderScreenBorderTransparency : !0,
        loaderScreenBorder0Opacity = typeof window.loaderScreenColor != 'undefined' ? loaderScreenColor : .9,
        loaderScreenBorder1Opacity = typeof window.loaderScreenColor != 'undefined' ? loaderScreenColor : .7,
        loaderScreenBorder2Opacity = typeof window.loaderScreenColor != 'undefined' ? loaderScreenColor : .5,
        loaderScreenColor = typeof window.loaderScreenColor != 'undefined' ? loaderScreenColor : '#0066FF',
        loaderScreenForeColor = typeof window.loaderScreenForeColor != 'undefined' ? loaderScreenForeColor : 'rgba(120, 120, 120, .5)',
            ...
            // Modification > Data
                ...
                data.innerHTML =
                    '<style media=all type=text/css>' +
                        ...
                        '}' +
                    '</style>' +
                    "<div style='animation: rotate 1s ease-in-out 0s infinite backwards; border: " + loaderScreenWidth + "px solid; border-bottom-color: " + (function() { let data = document.createElement('loader-screen-element'); data.style.color = loaderScreenColor; return loaderScreenBorderAlpha ? (loaderScreenBorderTransparency ? (data.style.color || getComputedStyle(data).getPropertyValue('color')).replace(/rgb\(/, 'rgba(').replace(/\)/, ', ' + loaderScreenBorder1Opacity + ')') : loaderScreenColor) : 'initial' })() + "; border-left-color: " + (function() { let data = document.createElement('loader-screen-element'); data.style.color = loaderScreenColor; return loaderScreenBorderAlpha ? (loaderScreenBorderTransparency ? (data.style.color || getComputedStyle(data).getPropertyValue('color')).replace(/rgb\(/, 'rgba(').replace(/\)/, ', ' + loaderScreenBorder2Opacity + ')') : loaderScreenColor) : 'initial' })() + "; border-right-color: " + (function() { let data = document.createElement('loader-screen-element'); data.style.color = loaderScreenColor; return loaderScreenBorderAlpha ? (loaderScreenBorderTransparency ? (data.style.color || getComputedStyle(data).getPropertyValue('color')).replace(/rgb\(/, 'rgba(').replace(/\)/, ', ' + loaderScreenBorder0Opacity + ')') : loaderScreenColor) : 'initial' })() + "; border-top-color: " + loaderScreenColor + "; border-radius: 50%; height: 75px; margin: 0 auto; margin-bottom: " + loaderScreenMargin + "px; opacity: " + loaderScreenOpacity + "; width: 75px'> </div>" +
                    "<small style='color: " + loaderScreenForeColor + "; font-family: \"Open Sans\", \"Calibri Light\", Calibri, sans-serif; font-size: " + loaderScreenFontSize + "px !important; margin: 0 auto; margin-top: " + loaderScreenMargin + "px; text-align: center'> " + loaderScreenMessage + " </small>";
